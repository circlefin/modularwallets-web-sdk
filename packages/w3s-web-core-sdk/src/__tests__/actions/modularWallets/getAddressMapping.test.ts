...
import { getAddressMapping } from '../../../actions/modularWallets'
...
describe('Actions > modularWallets > getAddressMapping', () => {
  ...
  it('should get address mapping with valid parameters', async () => {
    fetchMock.mockResponseOnce(
      getJsonRpcStringifyResponse(GetAddressMappingResult),
    )

    const response = await getAddressMapping(
      client,
      GetAddressMappingMockParameters,
    )

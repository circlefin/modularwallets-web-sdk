...
import {
  getMockOwners,
  LoginCredentialMock,
  MockDefaultWalletName,
  MockEoaAccount,
  MockISOString,
  toPasskeyTransport,
} from '../../../__mocks__'
import { toWebAuthnCredential } from '../../../accounts'
import { getDefaultWalletName } from '../../../accounts/implementations/getDefaultWalletName'
...
describe('Accounts > implementations > getDefaultWalletName', () => {
  it.each(owners)('should return mock result $description', ({ getOwner }) => {
    ...
    const walletName = getDefaultWalletName(owner)
    expect(walletName).toBe(MockDefaultWalletName[owner.type])
